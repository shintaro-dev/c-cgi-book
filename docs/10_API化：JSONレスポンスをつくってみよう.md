...
※ JSON文字列の手組みでは、構文ミス（カンマの付け忘れや余計な文字）によるエラーが起きやすくなります。  
必ず `[]` や `{}` の対応、文字列エスケープに注意してください。

※ JSONでは `"` や `\` を正しくエスケープする必要があります。  
HTMLと異なるため、専用の `json_escape()` を用意しています。

> ※ APIのテスト方法（curlコマンド、ブラウザ表示の注意など）については、  
> 第11章「Appendix：開発環境・デプロイTips集」にも紹介があります。